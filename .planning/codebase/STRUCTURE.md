...
- Frontend components/pages: PascalCase `.tsx` files (example `EventsPage.tsx`, `TraceViewPage.tsx`).
- Frontend hooks/utils/types: camelCase `.ts` files (example `useEvents.ts`, `timelineScale.ts`).
