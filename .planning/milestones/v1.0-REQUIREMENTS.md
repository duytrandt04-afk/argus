...
- [x] **DIAG-03**: `GET /api/version` returns app version, Git commit, and build date
- [x] **DIAG-04**: App version visible in frontend UI
- [x] **DIAG-05**: Startup emits actionable fatal error messages for: port already in use, DB path not writable, migration failure, invalid config
...
- [x] **REL-05**: App version injected at build time via ldflags (`git describe --tags --always`)
